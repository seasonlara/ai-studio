...
const creativeStyleFamilies = [
  ...
  {
    name: "食品零食包裝",
    keywords: ["食品", "零食", "坚果", "堅果", "腰果", "杏仁", "果乾", "果干", "蔓越莓", "餅乾", "饼干", "糖果", "咖啡", "茶", "飲品", "饮品", "麥片", "麦片", "燕麥", "燕麦"],
    style: "乾淨明亮的食品廣告感，使用原包裝主色、自然木質或淺色背景、真實食材陪襯、清楚包裝正面，畫面要有食慾但不可改包裝。",
    copy: "優先突出品類、口味、份量、食用場景與安心感；不得編造產地、健康功效、成分、重量、認證或保固。",
  },
  {
    name: "通用橙色爆款",
    keywords: [],
...
const evidenceRules = [
  ...
  "包装保真规则：如果上传图中有商品包装袋、包装盒、瓶罐、标签贴纸或品牌版面，必须把它当作商品身份的一部分严格保留；不得重新设计包装、替换品牌、改包装颜色、改袋型盒型、改标签位置、改口味名称或把包装换成另一款商品。",
  "包装文字规则：包装上的品牌、品名、口味、重量、图案和主色块要尽量沿用原图；如果模型无法准确复现小字，宁可弱化小字或保持模糊真实感，也不要生成与原图不同的新文字、新 Logo、新配色或新包装。",
  "食品包装规则：如果商品是坚果、零食、饮品、咖啡、茶、冲调、饼干等食品类，广告图可以增加碗、盘、食材或食用场景，但内容物必须与原图品类一致；不得把坚果画成其他食品，不得编造天然、无添加、健康功效、产地、克重或认证。",
...
function categoryGuard(settings) {
  ...
  const packagedFoodWords = ["食品", "零食", "坚果", "堅果", "腰果", "杏仁", "果乾", "果干", "蔓越莓", "餅乾", "饼干", "咖啡", "茶", "飲品", "饮品", "麥片", "麦片", "燕麥", "燕麦"];
  ...
  const isPackagedFood = packagedFoodWords.some((word) => text.includes(word));
  return [
    isStorage
    ...
    isPackagedFood
      ? "用户文字很可能是食品/零食/坚果类包装商品：必须以原包装正面作为商品识别核心，广告场景和配料陪衬可以美化，但包装袋/盒/瓶本身不得改款。"
      : "如果上传图包含包装类商品，即使用户标题未说明，也要优先保留原包装，不要重新设计包装。",
  ].join("\n");
}

function strictPackagingRules(settings) {
  if (!settings.strictPackagingMode) return "";

  return [
    "包装严格保真模式已开启：如果上传图中存在包装袋、包装盒、瓶罐、标签贴纸、品牌版面、口味名、净含量、配色块或产品正面图案，必须把它们视为商品身份本身，而不是可重新设计的广告素材。",
    "包装主体必须与上传图保持同款：袋型/盒型/瓶型、正面版式、主色、品牌位置、商品名称位置、口味/规格区域、图案元素、透明窗、封口、贴纸、图标和大色块都要尽量一致。",
    "禁止将原包装改成新包装、竞品包装、通用白袋、通用橙色包装、不同口味、不同品牌、不同容量、不同开口结构或不同标签版式。",
    "可以更换背景、灯光、桌面、场景、食材陪衬、人物手部、卖点标签和外部广告文案；但商品包装本体必须像从用户上传原图中抠出后重新拍摄。",
    "若模型无法精确生成包装小字，宁可让小字自然模糊、缩小或减少可读性，也不要编写与原图不同的新品牌、新口味、新克重、新功效或新认证。",
    "广告卖点请放在包装外部的信息标签、标题、角标或气泡中；不要把新卖点直接印到包装正面，避免破坏原包装。",
    "食品、保健品、美妆、日化、瓶罐、盒袋类商品尤其要遵守：不得编造天然、无添加、健康功效、产地、认证、保固、克重、适用症状或成分。",
    "如果上传商品不是包装类商品，则将本模式理解为产品身份严格保真：必须保留外形、颜色、材质、Logo/标签、按钮、接口、结构比例和关键细节，不要重绘成另一款商品。",
  ].join("\n");
}
...
function promptFor(item, settings) {
  ...
  const detailStrategy = detailBlueprint
    ...
    : "";
  const strictPackagingStrategy = strictPackagingRules(settings);
